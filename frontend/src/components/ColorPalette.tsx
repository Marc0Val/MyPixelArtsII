    ...
    <div className="flex flex-wrap gap-2 p-4 bg-white rounded-lg shadow-md">
      <button
        className={clsx(
          ...
          !selectedColor ? 'border-blue-500' : 'border-gray-200 hover:border-gray-300'
        )}
        onClick={() => setSelectedColor(null)}
        title="Modo navegación"
      >
        <Move size={16} className={!selectedColor ? 'text-blue-500' : 'text-gray-500'} />
      </button>
      {colors.map((color) => (
        <button
          key={color.id}
          className={clsx(
            ...
            selectedColor === color.hex ? 'border-blue-500' : 'border-gray-200 hover:border-gray-300'
          )}
